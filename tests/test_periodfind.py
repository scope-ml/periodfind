...
class TestStatistics:
    def test_construction(self):
        s = Statistics([1.0, 0.0], 0.5, mean=1.0, std=0.2,
                       median=1.0, mad=0.15)
    ...
    def test_significance_stdmean(self):
        s = Statistics([1.0], 3.0, mean=1.0, std=0.5,
                       median=1.0, mad=0.4)
        # |3.0 - 1.0| / 0.5 = 4.0
    ...
    def test_significance_madmedian(self):
        s = Statistics([1.0], 3.0, mean=1.0, std=0.5,
                       median=1.2, mad=0.3, significance_type='madmedian')
        # |3.0 - 1.2| / 0.3 = 6.0
        assert s.significance == pytest.approx(6.0)

    def test_significance_unknown_type(self):
        s = Statistics([1.0], 3.0, mean=1.0, std=0.5,
                       median=1.0, mad=0.4, significance_type='bogus')
        with pytest.raises(NotImplementedError):
            _ = s.significance

    def test_statistics_from_data_single(self):
        data = np.array([[1.0, 2.0, 3.0],
                         [4.0, 0.5, 6.0]], dtype=np.float32)
        params = [np.array([10.0, 20.0], dtype=np.float32),
    ...
    def test_statistics_from_data_precomputed_stats(self):
        ...
        s = Statistics.statistics_from_data(
            data, params, use_max=True, n=1,
            mean=100.0, std=50.0, median=99.0, mad=45.0)
...
def make_lsst_cadence(period, waveform='sinusoidal', n_years=10,
                      points_per_season=60, amplitude=1.0, noise_std=0.05,
                      seed=42):
    ...
    if waveform == 'sinusoidal':
        ...
        eclipse_width = 0.1
        in_eclipse = dist < eclipse_width / 2
        mags[in_eclipse] = (1.0 - amplitude
                            * (1.0 - dist[in_eclipse] / (eclipse_width / 2))
                            ).astype(np.float32)
...
class TestAOVOverlap:
    def test_overlap_differs_from_no_overlap(self):
        ...
        pgram_no = aov_no_overlap.calc([t], [m], periods, period_dts,
                                        output='periodogram')
        pgram_ov = aov_overlap.calc([t], [m], periods, period_dts,
                                     output='periodogram')
...
class TestLSSTScenarios:
    ...
    def test_lsst_cadence_sinusoidal(self):
        ...
        true_period = 5.0
        t, m = make_lsst_cadence(period=true_period, waveform='sinusoidal',
                                  n_years=10, points_per_season=80,
                                  amplitude=1.0, noise_std=0.05)
        periods = make_trial_periods(true_period, n_periods=500)
        period_dts = np.array([0.0], dtype=np.float32)
    ...
    def test_lsst_cadence_eclipsing_binary(self):
        ...
        true_period = 3.0
        t, m = make_lsst_cadence(period=true_period,
    ...
    def test_lsst_cadence_rr_lyrae(self):
        ...
        true_period = 0.55
        t, m = make_lsst_cadence(period=true_period, waveform='rr_lyrae',
                                  n_years=10, points_per_season=80,
                                  amplitude=0.8, noise_std=0.03)
        periods = make_trial_periods(true_period, n_periods=500)
        period_dts = np.array([0.0], dtype=np.float32)
...
class TestHarmonicsAndAliasing:
    ...
    def test_harmonics_detected(self):
        ...
        for AlgoCls in [ConditionalEntropy, AOV, LombScargle]:
            ...
            detected = result[0].params[0]
            assert period_matches(detected, true_period), \
                f"{AlgoCls.__name__}: detected {detected}, " \
                f"expected ~{true_period} or harmonic"
    ...
    def test_half_period_harmonic(self):
        ...
        assert period_matches(detected, true_period,
                              harmonics=[true_period / 2, 2 * true_period]), \
